...
```r
  ...
  sd2 <-  sd(filter(data, arm == "Fentanyl")$c_painscore10, na.rm = TRUE)
...
# Split the data by arm and cbind into dataset. 
  table_data <- cbind(
    ...
    ungroup() %>%
    select(-arm, -arm1, -time1)
  
  table_data <- full_join(
    table_data, 
    select(ttests_95, estimate, conf.low, conf.high, time), 
    by = "time"
  )
