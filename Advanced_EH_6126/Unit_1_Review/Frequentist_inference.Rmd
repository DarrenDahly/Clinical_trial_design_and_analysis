...
# Install/load packages
  ...
  packs <- c("tidyverse", "pander", "knitr", "viridis", "broom", "pwr")
...
So here we get a p-value of `r signif(pvalue, 3)` vs. `r signif(t_1$p.value, 2)` from the t-test. Of course the difference in means, the effect estimate, is exactly the same (`r round(t_1$estimate[1] - t_1$estimate[2], 2)`. What's different is the sampling distribution of the two estimators of that effect (t vs the regression coefficient).
...
As some of you might have expected, about 5% of the p-values fall below the nominal 5% level (`r round(table(cut(results$p_value, c(0, 0.05, 1)))[1] / k, 3)`)
...
Again, about 5% of the p-values fall below the nominal 5% level (`r   round(table(cut(results$p_value, c(0, 0.05, 1)))[1] / k, 3)`)
...
What just happened? Now if I use p = 0.05 to (correctly) reject the null, my power is `r round(table(cut(results$p_value, c(0, 0.05, 1)))[1] / k * 100, 3)`%, and my type 2 error rate is `r 100 - round(table(cut(results$p_value, c(0, 0.05, 1)))[1] / k * 100, 3)`%. 
